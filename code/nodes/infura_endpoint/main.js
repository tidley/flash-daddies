...
async function main() {
  await initStorage();
  // ETH
  tokenEth = await getEthBalance(new Token('', 'ETH'));
  tokenHolder.push(tokenEth);
  // DAI
  const contractDai = new web3.eth.Contract(JSON.parse(DAI_ABI), DAI_ADDRESS);
  let tokenDai = await getTokenBalance(new Token(contractDai, 'DAI'));
  tokenHolder.push(tokenDai);
  // WETH
  const contractWeth = new web3.eth.Contract(
    JSON.parse(WETH_ABI),
    WETH_ADDRESS,
  );
  let tokenWeth = await getTokenBalance(new Token(contractWeth, 'WETH'));
  tokenHolder.push(tokenWeth);
  // WBTC
  const contractWbtc = new web3.eth.Contract(
    JSON.parse(WBTC_ABI),
    WTBC_ADDRESS,
  );
  let tokenWbtc = await getTokenBalance(new Token(contractWbtc, 'WBTC'));
  tokenHolder.push(tokenWbtc);
  // COMPOUND
  const contractComp = new web3.eth.Contract(
    JSON.parse(COMP_ABI),
    COMP_ADDRESS,
  );
  let tokenComp = await getTokenBalance(new Token(contractComp, 'COMP'));
  tokenHolder.push(tokenComp);
  balInit = false;
...
async function getTokenBalance(tokenInfo) {
  try {
    tokenInfo.currentBal = await tokenInfo.contract.methods
      .balanceOf(UNLOCK)
      .call();
    if (balInit) {
      tokenInfo.startBal = tokenInfo.currentBal;
    ...
    tokenInfo.profit +=
      parseInt(tokenInfo.currentBal) - parseInt(tokenInfo.startBal);
    console.log(`${tokenInfo.label} balance:`, tokenInfo.currentBal);
    console.log(`${tokenInfo.label} profit:`, tokenInfo.profit);
    return tokenInfo;
  } catch (error) {
